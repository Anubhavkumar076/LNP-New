...
public class AdminRetailerVerification extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_admin_retailer_verification);
        mEditText = findViewById(R.id.admin_retailer_verification_search);
        searchButton = findViewById(R.id.admin_retailer_verification_search_button);

        mRecyclerView = findViewById(R.id.admin_retailer_verification);
        ...
        new Thread(() -> {
            try (Connection connection = DriverManager.getConnection(URL, USER, PASSWORD)) {
                String sql = "\n" +
                        ...
                        "lu.idlnp_user_id = lui.lnp_user_information_user_id where lu.lnp_user_admin  = 0\n" +
                        "and lui.lnp_user_information_pan_url is not null  and lui.lnp_user_information_aadhaar_url is not null";
                ...
                runOnUiThread(() -> {
                    AdminRetailerVerificationAdapter adapter = new AdminRetailerVerificationAdapter(AdminRetailerVerification.this, myListData);
            ...
            public void onClick(View view) {
                ...
                if(!TextUtils.isEmpty(name.trim())) {
                    if(name.trim().contains(" ")) {
                        ...
                        searchSql += " and lui.lnp_user_information_first_name like '%"+ firstName +"%' ";
                        searchSql += " and lui.lnp_user_information_second_name like '%"+ secondName +"%' ";
                    } else {
                        searchSql += " and lui.lnp_user_information_first_name like '%"+ name.trim() +"%' ";
                    }
                }
                ...
                new Thread(() -> {
                    try (Connection connection = DriverManager.getConnection(URL, USER, PASSWORD)) {
                        ...
                        runOnUiThread(() -> {
                            AdminRetailerVerificationAdapter adapter = new AdminRetailerVerificationAdapter(AdminRetailerVerification.this, myListData);
                            mRecyclerView.setHasFixedSize(true);
                            mRecyclerView.setLayoutManager(new LinearLayoutManager(AdminRetailerVerification.this));
                            mRecyclerView.setAdapter(adapter);
                        });
                    } catch (Exception e) {
                        Log.e("InfoAsyncTask", "Error reading school information", e);
